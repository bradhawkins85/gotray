...
strip debug metadata and remove absolute paths from the resulting binary. The
release tooling automatically keeps Windows binaries unstripped to avoid
`Trojan:Script/Wacatac.B!ml` false positives; set `GOTRAY_STRIP_SYMBOLS=1` to
re-enable stripping explicitly:
...
target other platforms. Symbol stripping is disabled automatically on Windows
to reduce antivirus false positives and can be overridden with
`GOTRAY_STRIP_SYMBOLS`. Additional linker arguments can be appended via
`GOTRAY_EXTRA_LDFLAGS`. Release automation can opt into UPX compression by
